<template>
    <div>
        ...
        <div class="main" v-else>
                            ...
                            <div class="price-block">
                                <div class="price-block-head">
                                    ...
                                    <div class="price-type">
                                        <div @click="priceSwitch($event, variant.id)"
                                            v-for="variant in product.variants"
                                            :class="{'price-type-val': true, 'active': variant.id == selected_var}"
                                            :data-type="variant.id">{{variant.title}}</div>

                                    </div>
                                </div>
                                <div class="price-block-val">
                                    <div :class="{'price-block-val__itm': true, 'active': variant.id == selected_var}"
                                        :data-type="variant.id" v-for="variant in product.variants">
                                        <span class="price price--old"  v-if="product.specs.includes('discounted')">1 590 ₽</span> <span
                                            class="price">{{variant.price}} ₽</span>
                                    </div>
                            ...
                            <div class="product-itm__btn">
                                <button aria-label="Сравнить" class="product-btn">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                        fill="none" data-src="../img/ico/solve.svg" class="svg replaced-svg">
                                ...
                                <button aria-label="В избранное" class="product-btn">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                        fill="none" style="--fill: #4E5359;" data-src="../img/ico/heart.svg"
                                        class="svg replaced-svg">
                                        <path
                                            d="M22.6207 9.36842L12 20.5481L1.37931 9.36842L7 3.45191L11.275 7.95191L12 8.71506L12.725 7.95191L17 3.45191L22.6207 9.36842Z"
                                            stroke="var(--fill)" stroke-width="2"></path>
                                    </svg>
                                </button>
                            </div>

                            <div class="catalog-info__btns">
                                <div :class="{'add-cart-btn': true, 'show-counter': inBasketInfo[product.variants.findIndex((elem) => elem.id == selected_var)].quantity > 0}">
                                    <button :class="{'btn js-add-cart': true}" aria-label="В корзину"
                                        @click="addToCart">В корзину</button>

                                    <div class="add-cart-counter">
                                        <div @click="changeQuantity" class="add-cart-counter__btn js-counter-btn"
                                            data-direction="minus"></div>
                                        <input type="text" :value="inBasketInfo[product.variants.findIndex((elem) => elem.id == selected_var)].quantity" class="add-cart-counter__input"
                                            aria-label="Количество" id="quantity_selector">
                                        <div @click="changeQuantity" class="add-cart-counter__btn plus js-counter-btn"
                                            data-direction="plus"></div>
                                    </div>
                        ...
                        <div class="show-more-text">
                            <div class="show-more-text__content">
                                <div class="catalog-desc">
                                    <h3>Описание</h3>
                                    <p>Ламинат Artens «Дуб Монгольский» предназначен для устройства напольного покрытия
                                        в жилых или торговых помещениях с высокой проходимостью (класс эксплуатации —
                                        33). Допускается применение для полов с подогревом (система «теплый пол») при
                                        температуре не выше +28°С. Материал изделия — ХДФ (древесноволокнистая плита
                                        высокой плотности).</p>
                                    <p>В одной упаковке весом 14,60 кг — 8 панелей размером 129,2 × 19,3 см и толщиной
                                        8 мм. Общая площадь панелей в одной упаковке — 1,99 м². Страна производства —
                                        Россия.</p>

                                    <h3>Преимущества</h3>
                                    <ul>
                                        <li>Торцы панелей оснащены специальными замками, позволяющими
                                            фиксировать их положение без клея — монтаж производится быстро и точно.</li>
                                        <li>Рисунок покрытия имитирует текстуру натуральной древесины.</li>
                                        <li>Поверхность материала защищена от повреждений, выдерживает значительные
                                            механические нагрузки, обеспечивая длительный срок службы изделия.</li>
                                    </ul>

                                    <h3>Дополнительная информация</h3>
                                    <p>Получить актуальные сертификаты на товар вы можете, обратившись напрямую
                                        в магазин.</p>
                                    <p>Внимание! Ламинированные полы передают рисунок древесины и, как древесина, могут
                                        отличаться нерегулярным рисунком и его оттенками, появлением сучков разных
                                        размеров и окраски, что не является дефектом. При монтаже чередуйте между собой
                                        панели разных оттенков из разных пачек. Не требуется подгонять рисунок дерева,
                                        декор предполагает хаотичную раскладку, для максимальной имитации натурального
                                        дерева. Цветопередача зависит от индивидуальных настроек вашего устройства. Цвет
                                        товара на экране может отличаться от реального.</p>
                                </div>
                            </div>
                        </div>
                        <div class="show-more-btn"><span class="text-more">Открыть полностью</span><span
                                class="text-lese">Закрыть</span></div>
                    </div>
                </section>
                            ...
                            <div class="show-more-text__content">
                                <div class="catalog-char">
                                    ...
                                    <div class="catalog-char-line">
                                        ...
                                        <div class="catalog-char-line__val">
                                            Россия
                                            <img alt="" class="catalog-char-line__val__ico"
                                                src="../img/ico/flag-ru.svg">
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <section class="section-margin js-scroll-section" id="catalog-buy">
                    ...
                    <div class="product-slider owl-loaded owl-drag">
                        <div class="owl-stage-outer">
                            <div class="owl-stage"
                                style="transform: translate3d(0px, 0px, 0px); transition: all 0s ease 0s; width: 2731px;">
                                <div class="owl-item active" style="width: 279.438px; margin-right: 24px;" v-for="rel_product in product.related" :key="rel_product.id">
                                    <product-card :product="rel_product"></product-card>
                                </div>
                            </div>
                        </div>
                ...
                <section class="section-margin js-scroll-section" id="catalog-related">
                    ...
                    <div class="product-slider owl-loaded owl-drag">
                        ...
                        <div class="owl-stage-outer">
                            <div class="owl-stage"
                                style="transform: translate3d(0px, 0px, 0px); transition: all 0s ease 0s; width: 2731px;">
                                <div class="owl-item active" style="width: 279.438px; margin-right: 24px;">
                                    <div class="product-itm @@class" data-num="@@num">
                                        <a aria-label="Детальная страница товара" class="product-itm__link"
                                            href="/"></a>
                                        <a class="product-itm-head" href="/">
                                            <div class="product-itm-images init" data-page="1">
                                                <div class="product-itm-images-stage"
                                                    style="transform: translateX(-234.844px);">

                                                    <div class="product-itm-images__itm">
                                                        <img alt="" class="fill-img" loading="lazy"
                                                            src="../img/tmp/product.jpg">
                                                    </div>
                                                    <div class="product-itm-images__itm">
                                                        <img alt="" class="fill-img" loading="lazy"
                                                    ...
                                                    <div class="product-itm-images__itm">
                                                        <img alt="" class="fill-img" loading="lazy"
                                                            src="../img/tmp/banner-bg.jpg">
                                                    </div>

                                                </div>
                                                <div class="product-itm-images-dots"><span
                                                        class="product-itm-dot"></span><span
                                                        class="product-itm-dot active"></span><span
                                                        class="product-itm-dot"></span></div>
                                            </div>
                                            ...
                                            <div class="product-itm-label-list">
                                                ...
                                                <span class="product-itm-label">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                        viewBox="0 0 16 16" fill="none" data-src="../img/ico/like.svg"
                                                        class="product-itm-label__ico svg replaced-svg">
                                                        <path fill-rule="evenodd" clip-rule="evenodd"
                                                            d="M4 8L7.375 3L9.0625 4.11111V6.88889H12.4375L13 8L11.5938 13H6.8125L5.125 12.4444H4V8ZM5.125 11.3333H5.30756L6.99506 11.8889H10.7387L11.8002 8.11453L11.7422 8H7.9375V4.70576L7.68702 4.54083L5.125 8.33642V11.3333Z"
                                                            fill="#F5333F"></path>
                                                        <path fill-rule="evenodd" clip-rule="evenodd"
                                                            d="M3.8 7.27273H3.2V11.7273H3.8V7.27273ZM2 6V13H5V6H2Z"
                                                            fill="#F5333F"></path>
                                                    </svg>
                                                ...
                                                <span
                                                    class="product-itm-label product-itm-label--main product-itm-label--green">
                                        ...
                                        <div class="product-itm-body">
                                            ...
                                            <div class="star-line">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                    viewBox="0 0 16 16" fill="none" data-src="../img/ico/star-half.svg"
                                                    class="star star--full svg replaced-svg">
                                                    <path
                                                        ...
                                                        fill="url(#paint0_linear_6388_53053)"></path>
                                                    ...
                                                    <defs>
                                                        <linearGradient id="paint0_linear_6388_53053" x1="5.56874"
                                                            y1="16.476" x2="10.4116" y2="-0.47583"
                                                ...
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                    viewBox="0 0 16 16" fill="none" data-src="../img/ico/star-half.svg"
                                                    class="star star--empty svg replaced-svg">
                                                    <path
                                                        d="M8 0L10.2571 4.89337L15.6085 5.52786L11.6521 9.18663L12.7023 14.4721L8 11.84L3.29772 14.4721L4.34794 9.18663L0.391548 5.52786L5.7429 4.89337L8 0Z"
                                                        fill="url(#paint0_linear_6388_53053)"></path>
                                                    <g clip-path="url(#clip0_6388_53053)">
                                            ...
                                            <div class="price-block">
                                                <div class="price-block-head">
                                                    <span>Цена за</span>
                                                    <div class="price-type">
                                                        <div class="price-type-val active" data-type="m2">м<sup>2</sup>
                                                        </div>
                                                ...
                                                <div class="price-block-val">
                                                    <div class="price-block-val__itm active" data-type="m2">
                                                        <span class="price price--old">1 590 ₽</span> <span
                                                            class="price">986 ₽</span>
                                                    </div>
                                                    <div class="price-block-val__itm" data-type="pack">
                                                        <span class="price price--old">3 590 ₽</span> <span
                                                            class="price">2 186 ₽</span>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="product-itm__btn">
                                                <div class="add-cart-btn">
                                                    <button class="btn js-add-cart"
                                                        aria-label="В корзину">В&nbsp;корзину</button>

                                                    <div class="add-cart-counter">
                                                        <div class="add-cart-counter__btn js-counter-btn"
                                                            data-direction="minus"></div>
                                                        <input type="text" value="1" class="add-cart-counter__input"
                                                            aria-label="Количество">
                                                        <div class="add-cart-counter__btn plus js-counter-btn"
                                                            data-direction="plus"></div>
                                                    </div>
                                                </div>
                                                ...
                                                <button aria-label="Сравнить" class="product-btn">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                        viewBox="0 0 24 24" fill="none" data-src="../img/ico/solve.svg"
                                                        class="svg replaced-svg">
                                                        <rect x="3" y="7" width="2" height="14" fill="#4E5359"></rect>
                                                        <rect x="18" y="7" width="2" height="14" fill="#4E5359"></rect>
                                                        <rect x="8" y="3" width="2" height="18" fill="#4E5359"></rect>
                                                ...
                                                <button aria-label="В избранное" class="product-btn">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                        viewBox="0 0 24 24" fill="none" style="--fill: #4E5359;"
                                                        data-src="../img/ico/heart.svg" class="svg replaced-svg">
                                                        <path
                                                            d="M22.6207 9.36842L12 20.5481L1.37931 9.36842L7 3.45191L11.275 7.95191L12 8.71506L12.725 7.95191L17 3.45191L22.6207 9.36842Z"
                                                            stroke="var(--fill)" stroke-width="2"></path>
                                                    </svg>
                                    ...
                                    <div class="product-itm @@class" data-num="@@num">
                                        ...
                                        <a class="product-itm-head" href="/">
                                            <div class="product-itm-images init" data-page="0">
                                                ...
                                                <div class="product-itm-images-dots"><span
                                                        class="product-itm-dot active"></span></div>
                                            </div>
                                            ...
                                            <div class="product-itm-label-list">
                                                ...
                                                <span
                                                    class="product-itm-label product-itm-label--main product-itm-label--promo">
                                                ...
                                                <span class="product-itm-label">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                        ...
                                                        data-src="../img/ico/star-half.svg"
                                                        class="product-itm-label__ico svg star star--full  replaced-svg">
                                                        <path
                                                            d="M8 0L10.2571 4.89337L15.6085 5.52786L11.6521 9.18663L12.7023 14.4721L8 11.84L3.29772 14.4721L4.34794 9.18663L0.391548 5.52786L5.7429 4.89337L8 0Z"
                                                            fill="url(#paint0_linear_6388_53053)"></path>
                                                        <g clip-path="url(#clip0_6388_53053)">
                                    ...
                                    <div class="product-itm @@class" data-num="@@num">
                                        ...
                                        <a class="product-itm-head" href="/">
                                            <div class="product-itm-images init" data-page="2">
                                                <div class="product-itm-images-stage"
                                                    style="transform: translateX(-469.688px);">
                                            ...
                                            <div class="product-itm-label-list">
                                                ...
                                                <span
                                                    class="product-itm-label product-itm-label--dark product-itm-label--main">
                                                    <span class="product-itm-label__text">—&nbsp;35%</span>
                                                </span>
                                            ...
                                            <span class="product-itm-buy-type">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                    viewBox="0 0 24 24" fill="none" data-src="../img/ico/roll-ico.svg"
                                                    class="svg replaced-svg">
                                                    <path fill-rule="evenodd" clip-rule="evenodd"
                                        ...
                                        <div class="product-itm-body">
                                            ...
                                            <h5 class="product-itm__name">ARBITON LM60 заглушка правая для плинтуса 77
                                                Песчаный дуб</h5>
                                            <div class="star-line">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                    viewBox="0 0 16 16" fill="none" data-src="../img/ico/star-half.svg"
                                                ...
                                                <div class="product-itm__desc__dop">
                                                    ...
                                                    <div class="product-itm__dop-attr">
                                                        ...
                                                        <div class="product-itm__dop-attr__itm">
                                                            ...
                                                            <div class="product-itm__dop-attr__itm__val">
                                                                80&nbsp;м<sup>2</sup></div>
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                <div class="owl-item" style="width: 279.438px; margin-right: 24px;">
                                    <div class="product-itm @@class" data-num="@@num">
                                        <a aria-label="Детальная страница товара" class="product-itm__link"
                                            href="/"></a>
                                        <a class="product-itm-head" href="/">
                ...
                <section class="section-margin js-scroll-section" id="catalog-review">
                    ...
                    <div class="catalog-review">
                        <div class="catalog-review__rating">
                            ...
                            <a class="btn catalog-review__rating__btn"
                                @click="reviewModalShow ? reviewModalShow = false : reviewModalShow = true">Написать
                                отзыв</a>
                        ...
                        <div class="catalog-review__content">
                            ...
                            <product-review v-for="review in reviews" :review="review"></product-review>
                ...
                <section class="section-margin js-scroll-section" id="catalog-doc">
                    ...
                    <div class="doc-list">
                        ...
                        <a class="doc-list-itm" href="/">
                            ...
                            </span> <span class="doc-list-itm__name">ТТХ ПВХ плитка (LVT) Forbo EFFEKTA STANDART
                                Classic</span>
                        </a>
                        <a class="doc-list-itm" href="/">
                            <span class="doc-list-itm__ico">
                            ...
                            <span class="doc-list-itm__name">Инструкция по укладке (замковое покрытие) ПВХ плитка (LVT)
                                Forbo EFFEKTA STANDART Classic</span>
                        </a>
                        ...
                        <div class="calc-list">
                            <div class="calc-list-itm">
                                ...
                                <div class="calc-list-itm__text">
                                    ...
                                    <div class="calc-list-itm__title h4">Кварцвиниловая ПВХ плитка (LVT) Forbo EFFEKTA
                                        STANDART Classic Granite</div>
                                </div>
                        ...
                        <form action="" class="form-calc js-form-calc">
                            ...
                            <div class="form-calc-head">
                                ...
                                <div data-show-type="size">
                                    <div class="btn btn--line js-change-calc-type" data-type="area">Указать площадь
                                    </div>
                                </div>
                                <div data-show-type="area">
                                    <div class="btn btn--line js-change-calc-type" data-type="size">Указать размеры
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="calc-cols">
                                ...
                                <label class="calc-cart">
                                    <input name="type" type="radio" value="Горизонтальное"> <span
                                        class="calc-cart__border"></span> <span class="calc-cart__ico">
                                ...
                                <label class="calc-cart">
                                    <input name="type" type="radio" value="Диагональное"> <span
                                        class="calc-cart__border"></span> <span class="calc-cart__ico">
                                    ...
                                    <div class="calc-result-grid__total">
                                        <div class="calc-result-table-total">
                                            <div class="add-cart-counter">
                                                <div class="add-cart-counter__btn js-counter-btn"
                                                    data-direction="minus"></div>
                                                <input aria-label="Количество" class="add-cart-counter__input"
                                                    type="text" value="1">
                                                <div class="add-cart-counter__btn plus js-counter-btn"
                                                    data-direction="plus"></div>
                                            </div>
            ...
            <modal-with-slot v-if="reviewModalShow" @modalclose="reviewModalShow = false">
                <review-form :productid="product.id" @modalclose="reviewModalShow = false"></review-form>
            </modal-with-slot>
...
import ReviewForm from "@/components/actions/ReviewForm.vue"
import ModalWithSlot from "@/components/actions/ModalWithSlot.vue"
import ProductCard from "@/components/ProductCard.vue"


export default {
    components: {
        ProductReview, ModalWithSlot, ReviewForm, ProductCard
    },
    data() {
        return {
            timeout: 0,
    ...
    methods: {
        ...
        async fetchReviews() {
            try {               
                const response = await axios.get(`${process.env.VUE_APP_BACKEND_URL}/product/get_reviews/${this.product['id']}`, { params: {
            ...
            response.data.forEach((element) => {
                if(element.images_urls) {
                    element.images_urls = JSON.parse(element.images_urls)
                }
            })
        ...
        checkInCart() {
            ...
            this.product.variants.forEach((variant, index) => {
                var index = basket.findIndex((prod) => prod.product_id == variant.id)
                if(index != -1) {
                    this.inBasketInfo.push({'quantity': basket[index].quantity,'variant_id': variant.id})
                } else {
                    this.inBasketInfo.push({'quantity': 0, 'variant_id': variant.id})
                }
            })
            ...
            this.product.related.forEach((rel, index) => {

                rel.inBasketInfo = []

                rel.variants.forEach((rel_var) => {
                    var index = basket.findIndex((prod) => prod.product_id == rel_var.id)

                    if(index != -1) {
                        rel.inBasketInfo.push({'quantity': basket[index].quantity, 'variant_id': rel_var.id})
                    }
                })
            })
